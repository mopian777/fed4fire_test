...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 client_fedasync.py <client_id> [server_ip|auto]")
        sys.exit(1)
    cid = sys.argv[1]
    ...
    for k in range(1, UPDATES + 1):
        ...
        # push update
        send_json(s, {"type":"UPDATE","k":k,"base_version":base_version,"w":w2,"b":b2,"train_time_ms":train_ms})

        # receive global model
        g = recv_json_line(f)
        if not g or g.get("type") != "GLOBAL":
            print(f"[{METHOD} {cid}] server closed.")
            break
        ...
        # eval global on local data, send back for server logging
        ...
        if PING_ENABLED:
            ...
            with open(ping_path,"a") as fp:
                fp.write(f"{METHOD},{cid},{host},{k},{sent},{rec},{pl:.6f},{rmin:.6f},{ravg:.6f},{rmax:.6f},{delay:.6f}\n")
