#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
RL-FedAvg server (jFed-style) with simple epsilon-greedy bandit (no torch).

- Listens on 0.0.0.0:5000 and accepts exactly 2 client connections.
- Each client must first send: {"type": "hello", "client_id": "client1" | "client2"}.
- In each round, the bandit agent chooses one of 3 actions:
    0 -> use {client1, client2}
    1 -> use {client1}
    2 -> use {client2}

- For selected clients, server sends:
    {
      "type": "train",
      "round": r,
      "weights": {"w": ..., "b": ...},
      "local_epochs": 5,
      "lr": 0.05
    }
  Clients locally train and reply:
    {
      "type": "update",
      "round": r,
      "client_id": "client1",
      "weights": {"w": ..., "b": ...},
      "local_loss": 0.0087
    }

- Server measures per-client round time = (recv_time - send_time) in ms.
- Reward: r = - (alpha_delay * round_time_ms + beta_loss * approx_global_loss).
- Bandit updates Q[action] using incremental average (no torch/numpy).
"""

import json
import socket
import struct
import time
import random


# --------------------------------------------------------------------
#  Utilities: JSON over TCP (length-prefixed)
# --------------------------------------------------------------------

def send_json(sock, obj):
    data = json.dumps(obj).encode("utf-8")
    header = struct.pack("!I", len(data))  # 4-byte big-endian length
    sock.sendall(header + data)


def recv_exact(sock, n):
    buf = b""
    while len(buf) < n:
        chunk = sock.recv(n - len(buf))
        if not chunk:
            raise ConnectionError("Socket closed")
        buf += chunk
    return buf


def recv_json(sock):
    header = recv_exact(sock, 4)
    length, = struct.unpack("!I", header)
    data = recv_exact(sock, length)
    return json.loads(data.decode("utf-8"))


# --------------------------------------------------------------------
#  Simple Global Model: y = w x + b
# --------------------------------------------------------------------

class GlobalModel(object):
    def __init__(self, w=0.0, b=0.0):
        self.w = float(w)
        self.b = float(b)

    def to_dict(self):
        return {"w": self.w, "b": self.b}

    @staticmethod
    def from_dict(d):
        return GlobalModel(w=d.get("w", 0.0), b=d.get("b", 0.0))

    @staticmethod
    def fedavg(models):
        if not models:
            return GlobalModel()
        w_sum = 0.0
        b_sum = 0.0
        for m in models:
            w_sum += m.w
            b_sum += m.b
        n = float(len(models))
        return GlobalModel(w=w_sum / n, b=b_sum / n)


# --------------------------------------------------------------------
#  Epsilon-greedy multi-armed bandit (no state, no torch)
# --------------------------------------------------------------------

class BanditAgent(object):
    def __init__(self, n_actions, eps=0.1, alpha=0.1):
        self.n_actions = int(n_actions)
        self.eps = float(eps)       # exploration prob
        self.alpha = float(alpha)   # learning rate for Q update
        self.q_values = [0.0 for _ in range(self.n_actions)]
        self.counts = [0 for _ in range(self.n_actions)]

    def select_action(self):
        # epsilon-greedy
        if random.random() < self.eps:
            a = random.randint(0, self.n_actions - 1)
        else:
            max_q = max(self.q_values)
            candidates = [i for i, q in enumerate(self.q_values) if q == max_q]
            a = random.choice(candidates)
        return a

    def update(self, action, reward):
        a = int(action)
        r = float(reward)
        self.counts[a] += 1
        old_q = self.q_values[a]
        new_q = old_q + self.alpha * (r - old_q)
        self.q_values[a] = new_q

    def summary(self):
        parts = []
        for a in range(self.n_actions):
            parts.append("a{}: Q={:.4f}, n={}".format(a, self.q_values[a], self.counts[a]))
        return " | ".join(parts)


# --------------------------------------------------------------------
#  Client connection metadata
# --------------------------------------------------------------------

class ClientConn(object):
    def __init__(self, sock, addr, client_id):
        self.sock = sock
        self.addr = addr
        self.client_id = client_id
        self.last_delay_ms = 0.7
        self.last_local_loss = 1.0


# --------------------------------------------------------------------
#  Main RL-FedAvg server with bandit-based selection
# --------------------------------------------------------------------

def run_server(
    host="0.0.0.0",
    port=5000,
    n_rounds=200,
    local_epochs=5,
    lr_local=0.05,
    alpha_delay=1.0,
    beta_loss=0.5,
):
    print("[SERVER] Listening on {}:{} ...".format(host, port))
    server_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    server_sock.bind((host, port))
    server_sock.listen(2)

    # Accept 2 clients
    clients = {}
    while len(clients) < 2:
        sock, addr = server_sock.accept()
        print("[SERVER] Connection from {}".format((addr,)))
        try:
            msg = recv_json(sock)
        except Exception as e:
            print("[SERVER] Failed to receive hello:", e)
            sock.close()
            continue

        if msg.get("type") != "hello":
            print("[SERVER] First message not 'hello', closing.")
            sock.close()
            continue

        cid = msg.get("client_id")
        if cid in clients:
            print("[SERVER] Duplicate client_id={}, closing.".format(cid))
            sock.close()
            continue

        clients[cid] = ClientConn(sock=sock, addr=addr, client_id=cid)
        print("[SERVER] Registered client_id={} from {}".format(cid, addr))

    if not ("client1" in clients and "client2" in clients):
        print("[SERVER] Expecting client1 and client2; got:", list(clients.keys()))
        return

    c1 = clients["client1"]
    c2 = clients["client2"]

    # Init global model
    global_model = GlobalModel(w=0.0, b=0.0)

    # Bandit agent with 3 actions: {1,2}, {1}, {2}
    bandit = BanditAgent(n_actions=3, eps=0.1, alpha=0.1)
    last_global_loss = 1.0  # for logging

    def select_subset(action_idx):
        if action_idx == 0:
            return [c1, c2]
        elif action_idx == 1:
            return [c1]
        else:
            return [c2]

    for r in range(1, n_rounds + 1):
        action_idx = bandit.select_action()
        selected_clients = select_subset(action_idx)
        selected_ids = [c.client_id for c in selected_clients]

        print("\n[SERVER][round {}] action={}, selected={}, global_wb=({:.4f},{:.4f})"
              .format(r, action_idx, selected_ids, global_model.w, global_model.b))

        # Send train command
        send_times = {}
        for c in selected_clients:
            cmd = {
                "type": "train",
                "round": r,
                "weights": global_model.to_dict(),
                "local_epochs": local_epochs,
                "lr": lr_local,
            }
            send_json(c.sock, cmd)
            send_times[c.client_id] = time.time()

        # Receive updates
        updated_models = []
        local_losses = []
        delays_ms = []

        for c in selected_clients:
            try:
                msg = recv_json(c.sock)
            except Exception as e:
                print("[SERVER] Error receiving from {}: {}".format(c.client_id, e))
                continue

            if msg.get("type") != "update" or msg.get("round") != r:
                print("[SERVER] Unexpected msg from {}: {}".format(c.client_id, msg))
                continue

            now = time.time()
            delay_ms = (now - send_times[c.client_id]) * 1000.0
            c.last_delay_ms = delay_ms

            wdict = msg.get("weights", {})
            local_loss = float(msg.get("local_loss", 0.0))
            c.last_local_loss = local_loss

            updated_models.append(GlobalModel.from_dict(wdict))
            local_losses.append(local_loss)
            delays_ms.append(delay_ms)

            print("[SERVER] <- {}: delay={:.3f} ms, local_loss={:.6f}"
                  .format(c.client_id, delay_ms, local_loss))

        # Aggregate
        if updated_models:
            global_model = GlobalModel.fedavg(updated_models)
            if delays_ms:
                round_time_ms = max(delays_ms)
            else:
                round_time_ms = 0.0
            approx_global_loss = sum(local_losses) / float(len(local_losses))
        else:
            round_time_ms = 0.0
            approx_global_loss = last_global_loss

        last_global_loss = approx_global_loss

        # Reward: negative of (delay + beta * loss)
        reward = - (alpha_delay * round_time_ms + beta_loss * approx_global_loss)
        bandit.update(action_idx, reward)

        print("[SERVER][round {}] round_time={:.3f} ms, global_loss≈{:.6f}, reward={:.4f}"
              .format(r, round_time_ms, approx_global_loss, reward))
        print("[SERVER] Bandit Q-values:", bandit.summary())

    # Finish: notify clients
    stop_msg = {"type": "stop"}
    for c in clients.values():
        try:
            send_json(c.sock, stop_msg)
        except Exception:
            pass
        c.sock.close()

    server_sock.close()
    print("\n[SERVER] Training finished, sockets closed.")
    print("[SERVER] Final bandit stats:", bandit.summary())


if __name__ == "__main__":
    run_server(port=6000)

