...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 client_fedavg.py <client_id> [server_ip|auto]")
        sys.exit(1)
    cid = sys.argv[1]
    ...
    while True:
        ...
        if msg.get("type") == "TRAIN":
            ...
            t0 = time.time()
            nw, nb, local_loss, local_acc = train_steps(xs, ys, w, b)
            train_ms = (time.time()-t0)*1000.0

            send_json(s, {"type":"UPDATE","round":r,"w":nw,"b":nb,"train_time_ms":train_ms})
        ...
        elif msg.get("type") == "EVAL_GLOBAL":
            r = int(msg.get("round", 0))
            w = float(msg.get("w", 0.0))
            b = float(msg.get("b", 0.0))
            gl, ga = eval_metrics(xs, ys, w, b)
            send_json(s, {"type":"EVAL","round":r,"global_loss":gl,"global_acc":ga,"n_samples":len(xs)})
